...
def block24h():
    ...
    conn.commit()
    conn.close()
    log('Account blocked for 24 hours. Definitely have to wait 24 hours. Block reason 3')
    countDown(1440, 3)
...
def updateAccount():
    ...
    if userData is None:
        ...
        loginCount += 1
        logCount += 1
        log(f'Queuing attempt {loginCount} will be stopped when attempt {maxCount} is reached. This is stage {blockCount}. Total attempts {logCount}')
